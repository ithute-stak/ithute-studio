...
class StudioPdfProject(Base):
    ...
    object_index: Mapped[list] = mapped_column(
        JSONB, nullable=False, default=list, server_default="[]"
    )
    font_catalog: Mapped[list] = mapped_column(
        JSONB, nullable=False, default=list, server_default="[]"
    )
    document_metadata: Mapped[dict] = mapped_column(
        "metadata", JSONB, nullable=False, default=dict, server_default="{}"
    )
    security: Mapped[dict] = mapped_column(
        JSONB, nullable=False, default=dict, server_default="{}"
    )
    import_state: Mapped[dict] = mapped_column(
        JSONB, nullable=False, default=dict, server_default="{}"
    )
...
class StudioPdfRevision(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "project_id", "version", name="uq_pdf_revision_project_version"
        ),
    )
