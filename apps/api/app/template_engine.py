...
def resolve_node(node: dict[str, Any], data: dict[str, Any]) -> dict[str, Any]:
    ...
    if kind == "variable":
        return _text(format_value(get_path(data, str(attrs.get("path", ""))), attrs.get("format")))
    if kind == "dynamicTable":
        rows = get_path(data, str(attrs.get("collectionPath", "")))
        rows = rows if isinstance(rows, list) else []
        columns = attrs.get("columns") or []
        header = {"type": "tableRow", "content": [_cell(str(c.get("label", c.get("path", ""))), True) for c in columns]}
        content = [header]
        for row in rows:
            item = row if isinstance(row, dict) else {"value": row}
            content.append({"type": "tableRow", "content": [_cell(format_value(get_path(item, str(c.get("path", ""))), c.get("format"))) for c in columns]})
...
def resolve_doc_json(document_json: dict[str, Any] | None, data: dict[str, Any]) -> dict[str, Any]:
    doc = deepcopy(document_json or {"type": "doc", "content": []})
    return resolve_node(doc, data)


def generate_document(template: dict[str, Any], data: dict[str, Any], source: dict[str, Any] | None = None) -> dict[str, Any]:
