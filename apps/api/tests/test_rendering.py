from app.document_renderer import render_pdf, render_docx
...
def test_pdf_render():
    data=render_pdf(sample()); assert data.startswith(b'%PDF'); assert len(data)>1000

def test_docx_render():
    data=render_docx(sample()); assert data.startswith(b'PK'); assert len(data)>1000
